-- Create exam_roadmap table if it doesn't exist (it looks like it already exists from the schema)
-- Let's check the existing table and add any missing columns
...
-- Update trigger for exam_roadmap
CREATE TRIGGER update_exam_roadmap_updated_at
  BEFORE UPDATE ON exam_roadmap
  FOR EACH ROW
